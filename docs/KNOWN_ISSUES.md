...
| ENV-6 | A conta ativa do GitHub CLI **volta sozinha** para `victorlins-dev` no meio de sessões — aconteceu 3 vezes só na sessão do M2/M3, sempre pega antes do push (nunca chegou a vazar um push com a conta errada, mas por pouco). Causa provável: outra sessão/processo Claude Code na mesma máquina usando `gh auth switch` para o próprio projeto dele. | alta (recorrente) | aberto — mitigado por processo | **Toda sessão deve rodar `gh auth status` imediatamente antes de CADA push** (não só uma vez no início), e se necessário `gh auth switch --user Victor-Hugo-Soares && gh auth setup-git`. Considerar perguntar ao Victor se ele roda outra sessão do Claude Code concorrente na mesma máquina em outro projeto GitHub |
...
| R-16 | **`ROADMAP.md` M14 previa "relatório do dia operacional" (faturamento, ticket médio, mais vendidos, cancelamentos/descontos por operador) dentro da Fase D — não foi implementado.** O M14 real (`ACTIVE_PLAN.md`, `QA_LEDGER.md`) ficou só em fechamento de caixa/divergência; o gate de saída da Fase D não menciona relatórios (só "totais reconstruíveis; permissões e concorrência testadas; fechamento sem conta manual"), então a Fase D foi dada como completa mesmo assim — mas é uma lacuna real do escopo original do `ROADMAP.md`, não um esquecimento silencioso. | — | aberto | Fazer antes de prometer relatórios ao Victor: cabe no M19 (Fase E, "relatórios avançados") ou como um milestone curto próprio antes disso, se ele pedir faturamento do dia antes da Fase E. Toda a base de dados já existe (`ledger_entries`, `payments`, `order_items.cancel_*`) — é só consulta, sem schema novo. |
