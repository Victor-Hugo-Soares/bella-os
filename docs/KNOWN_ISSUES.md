...
|----|------|-----------|--------|------|
| ENV-1 | **Docker Desktop falha ao iniciar** ("Inference manager" não consegue remover socket em `AppData/Local/Docker/run/dockerInference`). Tentativas: encerrar processos, apagar sockets (bloqueado pelo sistema), `EnableInference=false` em `settings-store.json` (backup `.bak-bella-os`), reiniciar: daemon continuou sem responder ao fim da sessão. | alta para dev local | **aberto** — ação do Victor | Victor apaga a pasta `C:\Users\Loma\AppData\Local\Docker\run` com o Docker fechado e reinicia; ou reinstala o Docker Desktop (`RUNBOOK_DEV.md §7`). Enquanto isso, a **CI é a frente de integração** (verde no M0). Fallback: Postgres local via instalador oficial ou banco de dev no Railway |
