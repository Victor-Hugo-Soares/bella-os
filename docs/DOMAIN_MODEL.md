...
| `cash_sessions` | tenant_id, cash_register_id, status (`open/closed`), opened_by, opened_at, opening_float_cents, closed_by, closed_at | índice único parcial: um `open` por registradora |
| `cash_movements` | tenant_id, cash_session_id, type (`withdrawal(sangria)/deposit(suprimento)/adjustment`), method, amount_cents, reason, by_user_id | `adjustment` reservado no `CHECK`, sem endpoint ainda (M14, YAGNI) |
| `cash_divergences` | tenant_id, cash_session_id, method, expected_cents, counted_cents, difference_cents, reason?, acknowledged_by | nunca ajustada em silêncio; gravada só quando `counted ≠ expected` |
...
**Correção de 2026-09-10 (M14):** três ajustes na tabela acima em relação ao que estava documentado: (1) `cash_movements.type` não tem mais `sale` — vendas já ficam em `payments` (`cash_session_id`+`method`+`amount_cents`), uma linha de movimento redundante para cada venda só duplicaria dado sem necessidade, decisão já tomada no Gate de Plano do M13 #8 e só agora refletida aqui; (2) `cash_sessions` não tem `expected`/`counted` JSONB — como `payments`/`cash_movements` são append-only (nunca apagados), o "esperado por forma" é sempre recalculável sob demanda a partir deles, uma coluna JSONB seria um cache que pode ficar desatualizado sem motivo real para existir; (3) `blind_close` (esconder o esperado do operador até ele contar) não foi implementado — é uma decisão de UX sem tela ainda para usá-la; fica para quando a tela de fechamento de caixa existir de verdade (adicionar depois é uma migration simples, uma coluna booleana nova, sem dado para migrar).
