...
| Q6 | Como recebe pagamento hoje (maquininha própria, TEF, PIX estático, dinheiro)? | Define se registramos pagamento manualmente ou integramos PSP | **Registro manual** de pagamento pelo caixa no MVP; PIX dinâmico e pagamento pelo celular são pós-MVP |
| Q7 | Cozinha quer **papel** (impressora térmica) além da tela? | Impressão exige agente local; muda Fase E | KDS em tela primeiro; impressão desenhada (fila de impressão) mas implementada na Fase E |
